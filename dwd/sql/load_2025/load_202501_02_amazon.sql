...
WITH cleaned AS (
    SELECT DISTINCT
        ...
        CASE WHEN s.category_name_new IS NULL OR UPPER(TRIM(CAST(s.category_name_new AS CHAR))) IN ('', '-', '--', 'NULL', 'N/A', 'NA', 'NAN', 'NONE') THEN NULL ELSE TRIM(CAST(s.category_name_new AS CHAR)) END AS category_name
