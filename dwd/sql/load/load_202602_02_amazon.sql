...
SET @source_row_count = (SELECT COUNT(*) FROM ec_cross_border.amazonus_shopinfo_202602_sales);
SET @empty_shop_id_count = (
    SELECT COUNT(*) FROM ec_cross_border.amazonus_shopinfo_202602_sales
...
SET @valid_shop_count = (
    ...
    FROM ec_cross_border.amazonus_shopinfo_202602_sales
...
INSERT INTO etl_load_audit_2026_gj (
    ...
    '202602-2', 202602, 2, 'Amazon', 'amazonus_shopinfo_202602_sales',
    @source_row_count, @empty_shop_id_count, @valid_shop_count,
...
WHERE month_id = 202602 AND platform_id = 2;
...
WITH cleaned AS (
    SELECT DISTINCT
        ...
        CASE WHEN s.company_address IS NULL OR UPPER(TRIM(CAST(s.company_address AS CHAR))) IN ('', '-', '--', 'NULL', 'N/A', 'NA', 'NAN', 'NONE') THEN NULL ELSE TRIM(CAST(s.company_address AS CHAR)) END AS company_address,
        ...
        CASE WHEN s.category_id_new IS NULL OR UPPER(TRIM(CAST(s.category_id_new AS CHAR))) IN ('', '-', '--', 'NULL', 'N/A', 'NA', 'NAN', 'NONE') THEN NULL ELSE TRIM(CAST(s.category_id_new AS CHAR)) END AS category_id,
        CASE WHEN s.category_name_new IS NULL OR UPPER(TRIM(REGEXP_REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(CAST(s.category_name_new AS CHAR), CHAR(13), ''), CHAR(10), ''), CHAR(9), ' '), CONVERT(UNHEX('C2A0') USING utf8mb4), ' '), '[[:space:]]+', ' '))) IN ('', '-', '--', 'NULL', 'N/A', 'NA', 'NAN', 'NONE', '未分类') THEN NULL ELSE TRIM(REGEXP_REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(CAST(s.category_name_new AS CHAR), CHAR(13), ''), CHAR(10), ''), CHAR(9), ' '), CONVERT(UNHEX('C2A0') USING utf8mb4), ' '), '[[:space:]]+', ' ')) END AS category_name
    FROM ec_cross_border.amazonus_shopinfo_202602_sales s
    ...
    SELECT
        ...
        ROUND(SUM(sales_money_value * 7.2), 2) AS sales_money,
        SUBSTRING_INDEX(
            GROUP_CONCAT(
...
SELECT
    202602, 2, 'Amazon', a.shop_id, a.shop_name,
...
CALL sp_promote_dwd_batch_2026_gj(202602, 2);
