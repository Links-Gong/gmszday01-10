...
本目录是 202601-202606 的独立重建版本，不修改 `intership_result` 中的旧成果，也不覆盖旧数据库表。
...
- 本次不装载 2025 同期，同比字段保持 SQL `NULL`，看板显示“无同期基期”。
- 真实 0 保留；缺失值和非法数值不能替换成 0。
...
5. 运行 `dwd/sql/90_validate_dwd.sql` 和 `dwd/sql/91_validate_ozon_dedup.sql`。
...
- 不要 `TRUNCATE` 正式 DWD，也不需要重跑已经显示 `SUCCESS` 的批次。

## 结果归档

每层 `results` 目录都有应导出的查询清单。建议从数据库客户端导出 UTF-8 CSV，并保留 SQL 文件名作为结果文件名前缀。
